    ...
    <div className="w-full max-w-7xl mx-auto px-6 sm:px-12 pt-32 pb-24 text-left" id="heritage-detail-view-container">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-6 flex flex-col gap-5">
          ...
          {/* Interactive Music Site Audio Guide Panel */}
          <div className="p-6 rounded-3xl bg-[rgba(0,0,0,0.25)] backdrop-blur-md border border-[rgba(255,255,255,0.15)] flex flex-col gap-4 relative overflow-hidden mt-2 glass-panel-custom">
            <div className="absolute top-0 right-0 w-24 h-24 rounded-full bg-white/5 blur-xl pointer-events-none" />
            
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-white/15 flex items-center justify-center text-[#FFD54F]">
                  <Volume2 className="w-5 h-5 flex-shrink-0" />
                </div>
                <div>
                  <h4 className="font-sans font-black text-white text-sm">Site Audio Guide Tour</h4>
                  <p className="text-[11px] text-white/60 font-medium">Spoken details of {heritage.district}</p>
                </div>
              </div>

              {/* Mute and playback trigger buttons */}
              <div className="flex items-center gap-3">
                <button
                  onClick={() => setIsAudioMuted(!isAudioMuted)}
                  className="p-2 rounded-xl text-white/55 hover:text-white hover:bg-white/5 transition-all cursor-pointer"
                >
                  {isAudioMuted ? <VolumeX className="w-4 h-4 text-white/40" /> : <Volume2 className="w-4 h-4 text-white" />}
                </button>

                <button
                  onClick={() => setIsAudioPlaying(!isAudioPlaying)}
                  className="px-4 py-2 bg-white text-stone-900 shadow-md hover:bg-white/95 text-xs font-mono font-bold tracking-wider rounded-xl flex items-center gap-1.5 border border-white cursor-pointer active:scale-95 transition-all btn-primary-custom"
                >
                  {isAudioPlaying ? (
                    <>
                      <Pause className="w-3.5 h-3.5 fill-stone-900 text-stone-900" /> PAUSE
                    </>
                  ) : (
                    <>
                      <Play className="w-3.5 h-3.5 fill-stone-900 text-stone-900 translate-x-0.5" /> PLAY
                    </>
                  )}
                </button>
              </div>
            ...
            {/* Custom Interactive Spectrometer during mock audio playing */}
            <div className="flex items-center gap-2">
              <div className="flex-1 bg-white/10 h-1.5 rounded-full overflow-hidden relative">
                <div 
                  className="h-full bg-gradient-to-r from-[#C2185B] to-[#FFD54F] transition-all duration-300"
                  style={{ width: `${audioPlaybackProgress}%` }}
                />
              </div>
              <span className="font-mono text-[9px] text-white/40 w-8 text-right">
                {Math.floor(audioPlaybackProgress / 20)}:{(Math.floor(audioPlaybackProgress % 20)).toString().padStart(2, "0")}
              </span>
            </div>
          </div>
        </div>
