...
// API Route: Dynamic local music scanner for Tagbeats.mp3 folder
app.get("/api/music-files", (req, res) => {
  const musicDir = path.join(process.cwd(), "Tagbeats.mp3");
  ...
  try {
    ...
    const audioFiles = files
      ...
      .map((file, idx) => {
        ...
        return {
          ...
          title: friendlyTitle,
          fileName: file,
          url: `/Tagbeats.mp3/${encodeURIComponent(file)}`,
    ...
    console.error("Failed to read Tagbeats.mp3 directory dynamically:", err);
